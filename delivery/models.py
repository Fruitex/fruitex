...
class DeliveryBucket(models.Model):
  ...
  def _get_delivery_bucket_orders(self):
    return DeliveryBucketOrder.objects.filter(delivery_bucket__id=self.id)
  ...
  delivery_bucket_orders = property(_get_delivery_bucket_orders)
